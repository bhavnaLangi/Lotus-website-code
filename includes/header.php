<header id="header" class="header header-fixed">
    <div class="container-fluid clearfix">
        <div class="row">
            <div class="brand">
                <a href="#video-home" class="scrollTo">
                    <div class="brand-logo">
                        <img src="img/logo/logo-small.png" class="logo" alt="logo" />
                    </div>
                    <!-- <div class="brand-name">Lotus Developers</div> -->
                    </a>
                </div>
                <button class="nav-toggle-btn a-nav-toggle">
                    <span class="nav-toggle">
                        <span class="stick stick-1"></span>
                        <span class="stick stick-2"></span>
                        <span class="stick stick-3"></span>







                </span>







            </button>







        </div>







    </div>















    <div class="hide-menu a-nav-toggle"></div>















    <div class="menu">







        <div class="menu-main" id="accordion">







            <ul>





                <li>

                    <a href="#video-home" class="a-nav-toggle scrollTo" data-text="Home Page">Home Page</a>

                </li>



                <li>







                    <a href="#about" class="a-nav-toggle scrollTo" data-text="Company Overviews">Company Overview</a>







                </li>



                <li>







                    <a href="#philosophy" class="a-nav-toggle scrollTo"







                        data-text="Our Philosophy">Our Philosophy</a>







                </li>







                <li>







                    <a href="#Difference" class="a-nav-toggle scrollTo"







                        data-text="The Lotus Difference">The Lotus Difference







                    </a>







                </li>















                <li>







                    <a href="#residential" class="a-nav-toggle scrollTo"







                        data-text="Residential Developments">Residential Developments</a>







                </li>







                <li>







                    <a href="#commercial" class="a-nav-toggle scrollTo"







                        data-text="Commercial Developments">Commercial Developments</a>







                </li>







                <!-- <li>







                    <a href="#gall" class="a-nav-toggle"







                        data-text="Gallery">Gallery</a>







                </li> -->







                <li>







                    <a href="#footer" class="a-nav-toggle scrollTo"







                        data-text="Contact Us">Contact Us</a>







                </li>







            </ul>







        </div>







        <!-- <div class="menu-footer">







            <div class="text">







                <a href="#!">







                    <span class="icon"><i class="socicon-periscope"></i></span>







                    <span class="line">Lotus Tower, Road No. 12 A, Behind Juhu GymKhana Club, Juhu , Mumbai - 400049.</span>







                </a>







            </div>







            <div class="text">







                <a href="mailto: sales@lotusdevelopers.com">







                    <span class="icon"><i class="socicon-mail"></i></span>







                    <span class="line">sales@lotusdevelopers.com</span>







                </a>







            </div>







            <div class="text">







                <a href="tel:+91-998-750-9311">







                    <span class="icon"><i class="socicon-viber"></i></span>







                    <span class="line">+91-9987509311</span>







                </a>







            </div>















        </div> -->







    </div>







</header>